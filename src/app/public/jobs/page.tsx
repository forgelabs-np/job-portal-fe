...
const getCategoryFromJob = (job: JobListing): string => {
  const searchText =
    `${job.title} ${job.description} ${job.requiredSkills}`.toLowerCase();

  if (
    searchText.includes("healthcare") ||
    searchText.includes("nurse") ||
  ...
  if (
    ...
    searchText.includes("elderly") ||
    searchText.includes("care giving")
  )
    return "Caregiving";

  if (
    searchText.includes("construct") ||
    searchText.includes("welder") ||
    searchText.includes("builder")
  )
    return "Construction";
  ...
  if (
    searchText.includes("hospitality") ||
  ...
  if (
    searchText.includes("manufactur") ||
    searchText.includes("factory") ||
...
const getCompanyName = (job: JobListing): string => {
  ...
  for (let i = 0; i < Math.min(words.length, 15); i++) {
    if (
      words[i].includes("Corp") ||
      words[i].includes("Ltd") ||
      words[i].includes("Company") ||
      words[i].includes("Inc")
    ) {
      return words[i];
    }
  }
...
export default function JobsPage() {
  ...
  const filteredJobs = useMemo(() => {
    ...
    if (filters.category !== "All") {
      filtered = filtered.filter(
        (job) =>
          getCategoryFromJob(job) === filters.category
      );
    }
