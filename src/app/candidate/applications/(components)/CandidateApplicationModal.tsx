...
import {
  ...
  useGetCandidateApplicationById,
...
type ApplicationStatus = "PENDING" | "APPROVED" | "REJECTED" | "SHORTLISTED" | "WITHDRAWN";
...
export const CandidateStatusBadge = ({ status }: { status: ApplicationStatus }) => {
...
const CandidateApplicationModal = ({
  applicationId,
  open,
  onClose,
}: CandidateApplicationModalProps) => {
  const { data: application, isLoading } = useGetCandidateApplicationById(
    applicationId ?? 0,
  );
                    ...
                    >
                      <HStack align="start" gap={2.5}>
                        ...
                        <Text fontSize="xs" color="blue.700" lineHeight="1.4">
                          <strong>Note:</strong> You cannot update your documents directly from this application. To replace or update a document (especially if it was rejected), please go to your <strong>Profile</strong> page and upload the corrected files.
                        </Text>
                      </HStack>
                    </Box>
