...
export interface InteractiveGlobeProps {
  className?: string;
  /** Max width/height hint in px; canvas fills its container when used inside a sized parent */
  size?: number;
  dotColor?: string;
  arcColor?: string;
  markerColor?: string;
  ...
  /** Radial glow color stop near the globe (canvas draw) */
  glowInner?: string;
  /** Radial glow color stop at the edge */
  glowOuter?: string;
  /** Globe circle outline stroke */
  rimColor?: string;
}
...
function labelFillColor(markerColor: string): string {
  return markerColor.replace(/,\s*[\d.]+\)$/, ", 0.6)");
}
...
export function InteractiveGlobe({
  ...
  glowInner = "rgba(60, 140, 255, 0.03)",
  glowOuter = "rgba(60, 140, 255, 0)",
  rimColor = "rgba(100, 180, 255, 0.06)",
  ...
  const draw = useCallback(() => {
    ...
    glowGrad.addColorStop(1, glowOuter);
    ...
    for (const marker of markers) {
      ...
      ctx.strokeStyle = markerColor.replace(/[\d.]+\)$/, `${0.2 + pulse * 0.15})`);
      ctx.lineWidth = 1;
      ctx.stroke();
      ...
      if (marker.label) {
        ...
        ctx.fillStyle = labelFillColor(markerColor);
...
/** shadcn-style default export name from the spec */
